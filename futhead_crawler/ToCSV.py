...
def switch2df(column, player_list:list)->pd.DataFrame:
        """
        ...
        """
        df = pd.DataFrame(player_list, columns=column)
        return df
def save_to_path(path,df):
        """
        保存球员信息表
        ...
        if not os.path.exists(path):
            os.mkdir(path)
        df.to_csv(path + '/players_info.csv',
                  index=False,
                  encoding="utf-8-sig")
        print('已保存至' + path + '\\players_info.csv')

def main():
    column = [
            'Name', 'Club', 'Nation', 'StrongFoot', 'Age','Height', 
            'Rating', 'PACE', 'SHOOTING', 'PASSING', 'DRIBBLING', 'DEFENCE',
            'PHYSICAL','acceleration','sprint_speed','positioning','finishing',
            'shot_power','long_shots','volleys','penalties','vision','crossing',
            'free_kick','short_passing','long_passing','curve','agility','balance',
            'reactions','ball_control','dribbling','composure','interceptions',
            'heading','def.awarness','standing_tackle','sliding_tackle','jumping',
            'stamina','strength','aggression'
        ]
    player_url_list = get_whole_player_url(1)
    info_list = get_player_infos(player_url_list)
    write_to_file(info_list)
    df = switch2df(column,info_list)
    save_to_path(os.getcwd(),df)
